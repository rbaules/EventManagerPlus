-- Correcciones incrementales posteriores a la aplicacion de 202608100002.
-- No modifica autorizacion operativa: evp_priv_usuario_tiene_acceso sigue exigiendo Activo.
...
begin
  ...
  if t.usr_estado not in ('Activo','Preregistrado') then
    raise exception using errcode='P0001',message='INVALID_MASTER_TARGET_STATUS';
  end if;
  ...
  return jsonb_build_object('ok',true,'codigo','OK','usuario_id',p_usuario_id,
    'es_master',true,'estado',t.usr_estado);
end; $$;
...
begin
  ...
  if p_estado='Inactivo' and t.usr_estado not in ('Activo','Preregistrado') then raise exception using errcode='P0001',message='INVALID_STATUS'; end if;
  if p_estado='Activo' and t.usr_estado<>'Inactivo' then raise exception using errcode='P0001',message='INVALID_STATUS'; end if;
  ...
  if t.usr_es_usuario_master and t.usr_estado='Activo' and p_estado='Inactivo'
